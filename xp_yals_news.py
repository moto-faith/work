...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '芦山县电子政务门户网'
        # 网站一级域名
        self.site_domain = 'yals.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.yals.gov.cn/xinwen/list/e9236da3-054a-4b94-b3c7-22677a2cf16c.html',#芦山要闻
            'http://www.yals.gov.cn/xinwen/list/0dcabc24-973e-4350-97db-9ca2c76a89b3.html',#部门动态
            'http://www.yals.gov.cn/xinwen/list/7535342e-8e96-41fb-b219-8b868c4f88e7.html',#乡镇动态
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="list_column list_column-page"]//li''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.yals.gov.cn", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//h1[@class="xq-title"]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        # ctime = data.xpath('''//div[@class="time pubtime"]/text()''').regex('\d+-\d+-\d+').datetime()
        ctime = data.xpath('''//meta[@name="PubDate"]/@content''').datetime()
        if ctime < self.c_time:
            return None
        ...
        content1 = data.xpathall('''//div[@class="xqing-web"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="xqing-web"]//img/@src''')
        if pic_urls_list:
