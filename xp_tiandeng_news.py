...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '广西天等政府网'
        # 网站一级域名
        self.site_domain = 'tiandeng.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.tiandeng.gov.cn/Home/Index/news/id/26',# 天等要闻
            'http://www.tiandeng.gov.cn/Home/Index/news/id/28',#部门（乡镇）动态
            'http://www.tiandeng.gov.cn/Home/Index/information.html'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            # loops = data.xpathall('''//div[@class="listtoitem"]//a''')

            for item in re.findall('"(/Home/Index/detailed.*?)"',data.response.text):  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                # if not post_url:
                #     continue
                post_url = urljoin("http://www.tiandeng.gov.cn", item)
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//h5[1]//text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="currenthot"]/a[3]//text()''').text().strip()
        ...
        retweeted_source = data.xpath('''//h5[2]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="listitem"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="listitem"]//img/@src''')
        if pic_urls_list:
