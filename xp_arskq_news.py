...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '阿尔山新闻网'
        # 网站一级域名
        self.site_domain = 'arskq.com'

        self.start_urls = [
            # 列表页链接
            'http://www.arskq.com/news/china', #国内新闻
            'http://www.arskq.com/news/world',  # 国际新闻
            'http://www.arskq.com/news/social',  # 社会新闻
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//span[@id="pubtime_baidu"]''').regex('\d+-\d+-\d+ \d+:\d+:\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        retweeted_source = data.xpath('''//span[@id="source_baidu"]//text()''').text().strip() or self.siteName
