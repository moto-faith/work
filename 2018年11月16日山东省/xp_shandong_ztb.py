...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "山东省公共资源交易网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://www.sdggzyjy.gov.cn"]
        self.encoding = 'utf-8'
        self.site_domain = 'sdggzyjy.gov.cn'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        titles = re.findall('class="article-list3-t">(.*?)</div>', unicode_html_body, re.S)
        links = re.findall('class="article-list3-t">.*?<a href="http://www.sdggzyjy.gov.cn:80/jsgczbgg/(.*?).jhtml"',unicode_html_body,re.S)
        dates = re.findall('<div class="list-times">(.*?)</div>',unicode_html_body,re.S)
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
        ...
        for title, link, date in zip(titles, links, dates):
            split_title = re.findall("</span>(.*?)<span style='color: red;'>", title, re.S)
            true_title = reduce(lambda x, y: x + y, split_title)
            url = 'http://192.168.20.139:9966/ggt/'+link
            code = requests.get(url).text
            link = 'http://www.sdggzyjy.gov.cn/jsgczbgg/{}.jhtml'.format(code)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:sdggzyjy", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:sdggzyjy", url)
            return False
...
if __name__ == '__main__':
    ...
    url = "http://www.sdggzyjy.gov.cn/queryContent-jyxxgk.jspx?channelId=78"
