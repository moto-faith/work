...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "上海公共资源平台"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://ggzy.shic.gov.cn"]
        self.encoding = 'utf-8'
        self.site_domain = 'ggzy.shic.gov.cn'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse(self, response, url):
        ...
        #     return ([], None, None)
        url_list = [
            'http://ggzy.shic.gov.cn/publicity/transaction/page?page=1&pageSize=10&columnStatus=1',
            'http://ggzy.shic.gov.cn/publicity/transaction/page?page=2&pageSize=10&columnStatus=1',
            'http://ggzy.shic.gov.cn/publicity/transaction/page?page=3&pageSize=10&columnStatus=1',
            # 政府采购

            'http://ggzy.shic.gov.cn/publicity/transaction/page?page=1&pageSize=10&columnStatus=3',
            'http://ggzy.shic.gov.cn/publicity/transaction/page?page=2&pageSize=10&columnStatus=3',
            'http://ggzy.shic.gov.cn/publicity/transaction/page?page=3&pageSize=10&columnStatus=3',
            # 工程建设
        ]
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        tag = "中标公告"
        titles = re.findall('"projectName":"(.*?)"', unicode_html_body, re.S)
        dates = re.findall('"time":"(.*?)"', unicode_html_body, re.S)
        links = re.findall('"projectCode":"(.*?)"', unicode_html_body, re.S)

        # li_content = data.xpathall('''//div[@class="list_zb02"]//tr''')
        ...
        #     date = item.xpath('''//td[5]/text()''').text ().strip ()
            ...
            if 'columnStatus=1' in from_tag_url:
                link = 'http://ggzy.shic.gov.cn/publicity/gp/winnerBulletin/{}'.format(link)
            else:
                link = 'http://ggzy.shic.gov.cn/publicity/constructionWinner/{}'.format(link)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:ggzyshic", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:ggzyshic", url)
            return False
