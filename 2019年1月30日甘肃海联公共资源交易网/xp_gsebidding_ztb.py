...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "甘肃海联公共资源交易网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://www.gsebidding.com"]
        self.encoding = 'utf-8'
        self.site_domain = 'gsebidding.com'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse(self, response, url):
        ...
        #     return ([], None, None)
        url_list = [
            'http://www.gsebidding.com/gszftz_web/jyxx/003001/003001001/',
            # 招标公告

            'http://www.gsebidding.com/gszftz_web/jyxx/003001/003001008/',
            # 变更公告

            'http://www.gsebidding.com/gszftz_web/jyxx/003001/003001004/',
            # 中标公告
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        li_content = data.xpathall('''//table[@class="sb-colu-bd"]//tr[@height="25"]''')
        ...
        for item in li_content:
            ...
            date = item.xpath('''//td[@align="right"]/span/text()''').text ().strip ()
            date = str(date).replace("-", "").replace("[", "20").replace("]", "")
            publicTime = date
            ...
            link = urljoin("http://www.gsebidding.com", link)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:gsebidding", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:gsebidding", url)
            return False
