...
class MySpider(spider.Spider):
    ...
    def parse(self, response, url):
        ...
        #     urls = []
        page_urls = []
        urls = self.db.table(self.result1).where('''tf = "1" and siteName = "甘肃海联公共资源交易网"''').find()
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        if data:
            ...
            contents = data.xpathall (
                '''//div[@class="sub-content"]//tr[3]//text()''')  # 内容
            content = ''
            for i in contents:
                content += i.text ().strip () + " "
            if content=='':
                content = data.xpath('''//div[@class="sub-content"]//tr[3]//img/@src''').text().strip()
                content = urljoin('http://www.gsebidding.com',content)
            if content=='':
...
if __name__ == '__main__':
    ...
    # ------------ parse_detail_page() ----------
    url = "http://www.gsebidding.com/gszftz_web/infodetail/?infoid=f46986d2-43ec-4c3c-ba7d-ebc306391b8a&categoryNum=003001004001"
    resp = spider.download(url)
