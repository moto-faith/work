...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '台州人才网'
        # 网站一级域名
        self.site_domain = 'tzrc.com'

        self.start_urls = [
            # 列表页链接
            'https://www.tzrc.com/News/Classes/5?page=1', #台州人事
            'https://www.tzrc.com/News/Classes/22?page=1',  # 人才开发
            'https://www.tzrc.com/News/Classes/28?page=1',  # 职场速递
            'https://www.tzrc.com/News/Classes/29?page=1',  # 薪资行情
            'https://www.tzrc.com/News/Classes/30?page=1' # 综合资讯
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="ulb"]/li/a''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if 'http' in post_url:
                    continue
                post_url = urljoin("https://www.tzrc.com", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="nr"]/h2//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="nr"]/p[@class="tm"]//text()''').replace('年','-').replace('月','-').replace('日','-').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//*[@id="bodyContentPane"]/div[2]/div[2]/h1/@title''').text().strip()
        ...
        content1 = data.xpathall('''//div[@class="xxs"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="xxs"]//img/@src''')
        if pic_urls_list:
