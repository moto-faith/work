...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '黑龙江水利'
        # 网站一级域名
        self.site_domain = 'hljsl.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.hljsl.gov.cn/channels/6.html',
            'http://www.hljsl.gov.cn/channels/9.html',
            'http://www.hljsl.gov.cn/channels/44.html',
            'http://www.hljsl.gov.cn/channels/45.html',
            'http://www.hljsl.gov.cn/channels/46.html'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@id="cona"]/ul/li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.hljsl.gov.cn", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//*[@id="cona"]/div/div/div/div[4]''')
        ...
        pic_urls_list = data.xpathall('''//*[@id="cona"]/div/div/div/div[4]//img/@src''')
        if pic_urls_list:
