...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '浙江健康教育馆'
        # 网站一级域名
        self.site_domain = 'zjhep.com'

        self.start_urls = [
            # 列表页链接
            'http://www.zjhep.com/index.php/Home/Index/cate/cate_id/12.html',#新闻快递
            'http://www.zjhep.com/index.php/Home/Index/cate/cate_id/13.html',#健康前沿
            'http://www.zjhep.com/index.php/Home/Index/cate/cate_id/14.html',# 工作动态
            'http://www.zjhep.com/index.php/Home/Index/cate/cate_id/47.html',#精选咨询
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            # loops = data.xpathall('''//div[@id="content"]//h3/a''')
            self.channel = data.xpath('''//li[@class="cate12"]/text()''').text().strip()
            for item in re.findall("href='(/index.php/Home/Index/detail.*?)'",data.response.text):  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                # if not post_url:
                #     continue
                post_url = urljoin('http://www.zjhep.com',item)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="content"]/div[@class="title"]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="summary"]/span[1]/text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        retweeted_source =data.xpath('''//div[@class="summary"]/span[@class="souce"]/text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="pageitem"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="pageitem"]//img/@src''')
        if pic_urls_list:
