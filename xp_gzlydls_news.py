...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '刘远东律师网'
        # 网站一级域名
        self.site_domain = 'gzlydls.com'

        self.start_urls = [
            # 列表页链接
            'http://www.gzlydls.com/h-col-105.html',
            'http://www.gzlydls.com/h-col-106.html'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="J_titleLine fk-titleLine"]''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.gzlydls.com/", post_url)
                # 模块内解析详情页时间，一般以//span、//div、//p等结尾，如没有则删除这三行
                # 例：ctime = item.xpath('''//div[@class="tail"]//span[1]''').datetime()
                ctime = item.xpath('''//a[@class="fk-newsListDate"]//text()''').datetime()
    ...
    def get_detail_page_info(self, data):
        ...
        # ctime = data.xpath('''//ul[@class="xinxi"]/li[1]''').regex('\d+-\d+-\d+ \d+:\d+').datetime()
        ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="newsGroup"]/a//text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@class="newsDetail newsDetailV2"]//p''')
        for item in content1:
            ...
            content_xml += item.data.encode('utf-8')
        ...
        pic_urls_list = data.xpathall('''//div[@class="newsDetail newsDetailV2"]//p//img/@src''')
        if pic_urls_list:
