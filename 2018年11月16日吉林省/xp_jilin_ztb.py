...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "吉林省公共资源交易信息网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://ggzyjy.jl.gov.cn"]
        self.encoding = 'gb2312'
        self.site_domain = 'ggzyjy.jl.gov.cn'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        li_content = data.xpathall('''//table[@id="DataList1"]//tr''')
        ...
        for item in li_content:
            ...
            date = item.xpath('''//td[@width="120"]/text()''').text ().strip ()
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
            ...
            link = "http://ggzyjy.jl.gov.cn/JiLinZtb"+link[5:]
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:jlggzy", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:jlggzy", url)
            return False
