...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '中山市国土资源局在线'
        # 网站一级域名
        self.site_domain = 'zsfdc.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.zsfdc.gov.cn/ArticleList.aspx?id=13', # 国土公告
            'http://www.zsfdc.gov.cn/ArticleList.aspx?id=14', # 国土动态
            'http://www.zsfdc.gov.cn/ArticleList.aspx?id=55', # 党风廉政建设
            'http://www.zsfdc.gov.cn/ArticleList.aspx?id=33', # 土地协议出让/土地国有划拨
            'http://www.zsfdc.gov.cn/ArticleList.aspx?id=32', #  房地产市场公告
            'http://www.zsfdc.gov.cn/ArticleList.aspx?id=65' # 不动产登记公告
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//table[@class="gridview"]//tr''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                post_url = item.xpath('''//td/a//@href''').text().strip()
                if not post_url:
                    continue
                post_url = urljoin("http://www.zsfdc.gov.cn/", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source = data.xpath('''//div[@class="textright"]/span[1]''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@id="article-content"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="article-content"]//img/@src''')
        if pic_urls_list:
