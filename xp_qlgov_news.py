...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '清廉网'
        # 网站一级域名
        self.site_domain = 'qlgov.org'

        self.start_urls = [
            # 列表页链接
            'http://www.qlgov.org/zixun/',
            'http://www.qlgov.org/toutiao/',
            'http://www.qlgov.org/fenxi/',
            'http://www.qlgov.org/shuji/',
            'http://www.qlgov.org/renwu/',
            'http://www.qlgov.org/faji/',
            'http://www.qlgov.org/fanfu/',
            'http://www.qlgov.org/lilun/',
            'http://www.qlgov.org/wenhua/',
            'http://www.qlgov.org/lishi/',
            'http://www.qlgov.org/huanqiu/',
            'http://www.qlgov.org/fupin/',
            'http://www.qlgov.org/difang/'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="ctitle"]''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.qlgov.org", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@class="azw"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="azw"]//img/@src''')
        if pic_urls_list:
