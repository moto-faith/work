...
class MySpider(spider.Spider):
    def __init__(self, cmd_args=None):
        ...
        self.siteName = "青龙山农场信息港"
        self.site_domain = 'qlsnjx.com'
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "01"
        self.start_urls = [
            'http://www.qlsnjx.com/html/list_533.html'
        ]
    ...
    def parse(self, response):
        ...
        if response is not None:
            ...
            urls = data.xpathall('''//div[@class="pingd"]//li//a''')
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        channel = data.xpath('''//div[@class="biaot-1"]''').text()
        title = data.xpath('''//div[@class="nytext"]''').text().strip()
        source = data.xpath('''//div[@class="nytext-1"]''').text().strip()
        ...
        ctime = data.xpath('''//div[@class="nytext-1"]''').regex('\d+年\d+月\d+日').datetime()
        if ctime < self.c_time:
            return
        content = data.xpath('''//div[@id="MyContent"]''').text().strip()
...
if __name__ == '__main__':
    ...
    url = 'http://www.qlsnjx.cn/html/list_533.html'
    resp = spider.download(url)
