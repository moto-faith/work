...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '兴仁县政府'
        # 网站一级域名
        self.site_domain = 'gzxr.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.gzxr.gov.cn/xwzx/zwyw/', #国内新闻
            'http://www.gzxr.gov.cn/xwzx/bmdt/',  # 国际新闻
            'http://www.gzxr.gov.cn/xwzx/xzdt/',  # 社会新闻
            'http://www.gzxr.gov.cn/xwzx/tpxw/',#图片新闻
            'http://www.gzxr.gov.cn/xwzx/ldhd/',#领导活动
            'http://www.gzxr.gov.cn/xwzx/tzgg/',#通知公告
            'http://www.gzxr.gov.cn/xxgk/'#信息公开
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            # loops = data.xpathall('''//div[2]/div/div[2]/div[2]//ul//a''')
            ...
            for item in re.findall('var str_1 = "(.*?)"',data.response.text):
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="nry_content t_c"]/h2//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//ul[@class="ul_list_a auto"]/li[2]/text()''').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="dqwz_box f14"]/a[3]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source = data.xpath('''//ul[@class="ul_list_a auto"]/li[4]//text()''').text().strip() or self.siteName


        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source = data.xpath('''//ul[@class="ul_list_a auto"]/li[5]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//font[@id="Zoom"]''')
        ...
        pic_urls_list = data.xpathall('''//font[@id="Zoom"]//img/@src''')
        if pic_urls_list:
