...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '相天网'
        # 网站一级域名
        self.site_domain = 'sz121.com'

        self.start_urls = [
            # 列表页链接
            'http://www.sz121.com/qxj/infodetail/newslist.aspx?tid=33&typername=%E8%A1%8C%E4%B8%9A%E5%8A%A8%E6%80%81', #行业动态
            'http://www.sz121.com/qxj/infodetail/newslist.aspx?tid=146&typername=%E9%87%8D%E8%A6%81%E5%A4%A9%E6%B0%94',  # 重要天气
            'http://www.sz121.com/qxj/infodetail/newslist.aspx?tid=144&typername=%E5%AA%92%E4%BD%93%E6%8A%A5%E9%81%93',  # 媒体报道
            'http://www.sz121.com/qxj/infodetail/newslist.aspx?tid=32&typername=%E8%8B%8F%E6%B0%94%E6%96%B0%E9%97%BB',  # 苏气新闻
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//a[@class="nblue"]''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = "http://www.sz121.com/qxj" +post_url[2:]
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="news_text"]/h1//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="news_textspan"]/span[2]//text()''').regex('\d+年\d+月\d日').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="cutPos"]//a[2]//text()''').text().strip()
        ...
        retweeted_source =data.xpath('''//div[@class="news_textspan"]/span[1]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="news_text"]/table''') or data.xpathall('''//div[@class="news_text"]/p''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="news_text"]//img/@src''')
        if pic_urls_list:
