...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '中国设计网'
        # 网站一级域名
        self.site_domain = 'cndesign.com'

        self.start_urls = [
            # 列表页链接
            'http://www.cndesign.com/domestic/', #国内资讯
            'http://www.cndesign.com/world/',  # 国际资讯
            'http://www.cndesign.com/activity/',#活动展览
            'http://www.cndesign.com/interview/',#人物访谈
            'http://www.cndesign.com/trend/',  #行业聚焦
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="content_box"]//h3''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.cndesign.com' , post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//h3[@id="newsTitle"]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="prod_hots"]/div[3]/span[@class="icon_time"]//text()''').regex('\d+-\d+-\d+').datetime()
        if ctime < self.c_time:
           return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="ph_lists_box"]/a[@rel="nofollow"]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source = data.xpath('''//div[@class="content_box"]/div[1]/div[1]/span[2]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@id="detail"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="detail"]//img/@src''')
        if pic_urls_list:
