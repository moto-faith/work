...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '广德县人民政府	'
        # 网站一级域名
        self.site_domain = 'guangde.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.guangde.gov.cn/content/channel/597ae371ceab06f01713e593/',#新闻中心
            'http://www.guangde.gov.cn/content/channel/597ae38aceab06a31813e593/',#广德要闻
            'http://www.guangde.gov.cn/content/channel/597ae3cdceab06541913e593/',# 部门动态
            'http://www.guangde.gov.cn/content/channel/597ae3deceab06b11a13e593/',#乡镇动态
            'http://www.guangde.gov.cn/content/channel/597af7c0ceab06207413e593/',#通知公告
            'http://www.guangde.gov.cn/content/channel/5a25ed647f8b9ab19a8f2685/',#要情专报
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="listright_box"]//li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.guangde.gov.cn',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        channel =data.xpath('''//div[@class="m_location"]/a[3]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source = data.xpath('''//font//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@id="zoom"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="zoom"]//img/@src''')
        if pic_urls_list:
