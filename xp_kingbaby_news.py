...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '婴之杰早教中心加盟'
        # 网站一级域名
        self.site_domain = 'kingbaby.com.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.kingbaby.com.cn/index.php?m=content&c=index&a=lists&catid=48',# 新闻与活动
            'http://www.kingbaby.com.cn/index.php?m=content&c=index&a=lists&catid=62',#每日见闻
            'http://www.kingbaby.com.cn/index.php?m=content&c=index&a=lists&catid=63',#最新动态
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.kingbaby.com.cn', post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="time tc"]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//div[@id="main_nav"]/a[2]//text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@class="newsinfo-intro"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="newsinfo-intro"]//img/@src''')
        if pic_urls_list:
