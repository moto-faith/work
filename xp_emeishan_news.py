...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '峨眉山市人民政府'
        # 网站一级域名
        self.site_domain = 'emeishan.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.emeishan.gov.cn/emss/ttxx/list.shtml',# 峨眉要闻
            'http://www.emeishan.gov.cn/emss/gwyxx/list.shtml',#国务院信息
            'http://www.emeishan.gov.cn/emss/szfxx/list.shtml',#省政府信息
            'http://www.emeishan.gov.cn/emss/bmdt/list.shtml',#部门动态
            'http://www.emeishan.gov.cn/emss/xzdt/list.shtml',#乡镇动态
            'http://www.emeishan.gov.cn/emss/gsgg/list.shtml',#通知公告
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="list_ul_a"]//a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = "http://www.emeishan.gov.cn"+ post_url[5:]
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source = data.xpath('''//div[@class="content_fz clearfix"]/span[1]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="pages_content"]''') or data.xpathall('''//div[@class="content"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="pages_content"]//img/@src''')or data.xpathall('''//div[@class="content"]//img/@src''')
        if pic_urls_list:
