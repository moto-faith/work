...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '汉滨区人民政府'
        # 网站一级域名
        self.site_domain = 'hanbin.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.hanbin.gov.cn/Node-138.html',# 汉滨要闻
            'http://www.hanbin.gov.cn/Node-139.html',#最新消息
            'http://www.hanbin.gov.cn/Node-140.html',#镇办动态
            'http://www.hanbin.gov.cn/Node-141.html',  #部门信息
            'http://www.hanbin.gov.cn/Node-142.html',#简讯快报
            'http://www.hanbin.gov.cn/Node-143.html',#社会热点
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="comm-list"]/li/a''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//span[@data-search="updatetime"]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//h3//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source =data.xpath('''//div[@class="other-info"]/span[1]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@id="fontzoom"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="fontzoom"]//img/@src''')
        if pic_urls_list:
