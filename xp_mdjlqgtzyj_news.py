...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '牡丹江林区国土资源局'
        # 网站一级域名
        self.site_domain = 'mdjlqgtzyj.gov.cn'

        self.start_urls = [

            # 列表页链接
            'http://www.mdjlqgtzyj.gov.cn/list.asp?class=72',
            'http://www.mdjlqgtzyj.gov.cn/list.asp?class=120'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="fenye_list"]/ul/li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.mdjlqgtzyj.gov.cn/", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@class="fenye_body_body"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="rightbox"]/p//img/@src''')
        if pic_urls_list:
