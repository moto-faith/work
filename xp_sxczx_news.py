...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '长治市上党区人民政府'
        # 网站一级域名
        self.site_domain = 'sxczx.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.sxczx.gov.cn/xwdt/zwyw/',#政务要闻
            'http://www.sxczx.gov.cn/xwdt/bmdt/',#部门动态
            'http://www.sxczx.gov.cn/xwdt/xzdt/',# 乡镇动态
            'http://www.sxczx.gov.cn/xwdt/snyw0/',#市内要闻
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="aon-con"]/ul/li/a''')
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source = data.xpath('''//div[@class="t"]/p/text()[4]''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''///div[@class="article-con"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="article-con"]//img/@src''')
        if pic_urls_list:
