...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '湖南大学自考成考信息咨询网'
        # 网站一级域名
        self.site_domain = 'hndxzk.org'

        self.start_urls = [

            # 列表页链接
            'http://www.hndxzk.org/xinwendongtai/',
            'http://www.hndxzk.org/xinwendongtai/xueyuanxinwen/',
            'http://www.hndxzk.org/xinwendongtai/zikaoxinwen/',
            'http://www.hndxzk.org/xinwendongtai/chengkaoxinwen/'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'gbk'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="new"]//li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.hndxzk.org", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="laiyuan"]//text()''').regex('\d+-\d+-\d+ \d+:\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="about_left1w"]/a[3]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source = data.xpath('''//div[@class="about_left1"]/a[3]//text()''').text().replace('来源：',
                                                                                      '').strip() or self.siteName

        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source = data.xpath('''//div[@class="about_left1"]/a[3]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="newye"]/p''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="newye"]//img/@src''')
        if pic_urls_list:
