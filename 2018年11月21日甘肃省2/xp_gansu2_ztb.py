...
class MySpider(spider.Spider):
    ...
    def parse(self, response, url):
        ...
        #     return ([], None, None)
        url_list = [
            'http://www.gansubidding.com/DataHandlers/PagerHandler.ashx?pageIndex=1&pageSize=50',
            #招标公告
            'http://www.gansubidding.com/DataHandlers/CorrectList.ashx?pageIndex=1&pageSize=50',
            #变更公告
            'http://www.gansubidding.com/DataHandlers/BiddingList.ashx?pageIndex=1&pageSize=50',
            # 工程建设中标公告
        ]
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
        ...
        for title,date,link in zip(titles,dates,links):
            ...
            link = 'http://www.gansubidding.com'+str(link)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:ggzyjy.gansu", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:ggzyjy.gansu", url)
            return False
