...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '田林县人民政府门户网站'
        # 网站一级域名
        self.site_domain = 'tianlin.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.tianlin.gov.cn/html/News/shizhengxinwen.html',# 时政新闻
            'http://www.tianlin.gov.cn/html/News/shehuiyaowen.html',#社会要闻
            'http://www.tianlin.gov.cn/html/News/bumendongtai.html',#部门动态
            'http://www.tianlin.gov.cn/html/News/xiangzhendongtai.html',  #镇办动态
            'http://www.tianlin.gov.cn/html/News/meitijujiao.html',  #通知公告
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="lismain"]//li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                if 'tianlin' not in post_url:
                    continue
                # post_url = urljoin("http://www.boshan.gov.cn", item)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//meta[@name="ArticleTite"]/@content''').text().strip()
