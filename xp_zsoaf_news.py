...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '舟山市海洋与渔业局网站'
        # 网站一级域名
        self.site_domain = 'zsoaf.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.zsoaf.gov.cn/SY002.html',#新闻动态
            'http://www.zsoaf.gov.cn/00001.html',#图片新闻
            'http://www.zsoaf.gov.cn/00002.html',#本地动态
            'http://www.zsoaf.gov.cn/00003.html',#省内动态
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//td[@align="left"]//a''')
            # self.channel = data.xpath('''//div[@class="title"]/span''').text().strip()
            for item in loops[:15]:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
    ...
    def get_detail_page_info(self, data):
        ...
        pic_urls_list = data.xpathall('''//div[@class="newsImg"]//img/@src''')
        if pic_urls_list:
