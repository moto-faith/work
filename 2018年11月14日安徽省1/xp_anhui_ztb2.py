...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        self.headers = {
            ...
            # "Cookie": "_gscu_1811078948=41663282vx6ent53; ASP.NET_SessionId=hin5jwidt3z0hdibmljlgw55; cookies=98184645; _gscbrs_1811078948=1; _gscs_1811078948=422661865igvey20|pv:1",
            "DNT": "1",
            "Host": "www.hfggzy.com",
    ...
    def parse(self, response, url):
        ...
        #     urls = []
        page_urls = []
        urls = self.db.table(self.result1).where('''tf = "1" and siteName = "安徽合肥公共资源交易中心"''').find()
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        if data:
            ...
            # content_xmls = se.xpath('//*[@id="TDContent"]')
            content_xml = ""
            for i in content_xmls:
                content_xml += i.data

            # 2招标（中标）内容
            contents = data.xpathall ('''//*//text()''')  # 内容
            # contents = se.xpath('//*[@id="TDContent"]//span/text()')
            content = ''
            for i in contents:
    ...
    def getPurchasingPersonName(self,content, content_xml):
        ...
        # 4采购人
        try:
            purchasing_person_name = re.findall("招标(采购)人名称：(.*?)<", content, re.M)[0]
        except:
            try:
    ...
    def getPurchasingPerson(self,content, content_xml):
        try:
            purchasing_person_name = re.findall ("第一中标（成交）候选人名称：(.*?)<", content, re.M)[0]
    ...
    def getprice(self,content, content_xml):
        try:
            purchasing_person_name = re.findall ("投标报价：(.*?)<", content_xml, re.M)[0]
        except:
            try:
...
if __name__ == '__main__':
    ...
    # ------------ parse() ----------
    print "开始登录"
    url = "http://www.hfggzy.com/hfzbtb/InfoDetail/Default.aspx?InfoID=a5be9377-0b28-4a9b-8682-efcf6729f284&CategoryNum=029023001002"
