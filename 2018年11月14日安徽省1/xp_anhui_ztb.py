...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        self.start_urls = ["http://www.hfggzy.com"]
        self.encoding = 'utf-8'
        self.site_domain = 'hfggzy.com'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse(self, response, url):
        ...
        #     return ([], None, None)
        url_list = [
            'http://www.hfggzy.com:7090/fulltextsearch/rest/getfulltextdata?format=json&rmk3=029023001002&pn=0&rn=20&cl=150',
            # 工程建设候选人公示

            'http://www.hfggzy.com:7090/fulltextsearch/rest/getfulltextdata?format=json&rmk3=029023001001&pn=0&rn=20&cl=150',
            # 工程建设中标结果

            'http://www.hfggzy.com:7090/fulltextsearch/rest/getfulltextdata?format=json&rmk3=029023002001&pn=0&rn=20&cl=150',
            # 政府采购中标公告
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        json_list = json.loads(unicode_html_body)['result']['records']

        for json_detail in json_list:

            title = json_detail['title']
            link = str(json_detail['link'])
            date = json_detail['date']
            tag = json_detail['remark1']
            date = str(date)[:10].replace("-","")

            # 网页跳转
            true_url = requests.get(link).text
            true_link = re.findall("window.location.href='(.*?)'",true_url)[0]
            link = "http://www.hfggzy.com"+true_link
            link = str(link)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:hfggzy", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:hfggzy", url)
            return False
...
if __name__ == '__main__':
    ...
    url = "http://www.hfggzy.com:7090/fulltextsearch/rest/getfulltextdata?format=json&rmk3=029023001002&pn=0&rn=20&cl=150"
