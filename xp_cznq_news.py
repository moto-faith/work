...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '滁州市南谯区人民政府官网'
        # 网站一级域名
        self.site_domain = 'cznq.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94cd/',# 南谯要闻
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94cc/',#部门动态
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94cb/',#乡镇传真
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94c6/',#视频新闻
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94c8/',  #媒体南谯
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94c7/',  #图片报道
            'http://www.cznq.gov.cn/content/channel/55893d88bb86f374b24e94c5/',  #通知公告
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="listNew"]/li''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.cznq.gov.cn", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="m-contentBox"]/h1//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="u-newsinfo"]/span[5]//text()''').regex('\d+-\d+-\d+ \d+:\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="m-location"]/a[3]//text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@class="u-newscontnet"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="u-newscontnet"]//img/@src''')
        if pic_urls_list:
