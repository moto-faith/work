...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '丽水福彩网'
        # 网站一级域名
        self.site_domain = 'lsflcp.cn'

        self.start_urls = [
            # 列表页链接
            'http://lsflcp.cn/fczx/fcdt/', #福彩动态
            'http://lsflcp.cn/fczx/zdfc/',  # 站点风采
            'http://lsflcp.cn/fczx/tbgg/',  # 特别公告
            'http://lsflcp.cn/fczx/spkj/', #视频开奖
            'http://lsflcp.cn/fczx/cssy/',#慈善事业
            'http://lsflcp.cn/fczx/csxw/', #彩市新闻
            'http://lsflcp.cn/fczx/ggrzx/' # 刮刮乐资讯
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//table[@class="mtop5"]//table//a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://lsflcp.cn",post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//tr[@valign="top"]''')
        ...
        pic_urls_list = data.xpathall('''//tr[@valign="top"]//img/@src''')
        if pic_urls_list:
