...
class MySpider(spiderDefault.Spider):
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="main"]/div[@class="col-left"]/ul''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                ctime = item.xpath('''//span[@class="rt"]''').datetime()
    ...
    def get_detail_page_info(self, data):
        ...
        channel = data.xpath('''//div[@class="crumbs"]/a[2]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        # source = data.xpath('''//div[@id="Article"]/h1/span//text()''').text().replace('来源：',
        ...
        # retweeted_source = data.xpath('''//div[@id="Article"]/h1/span//text()''').text().strip() or self.siteName
