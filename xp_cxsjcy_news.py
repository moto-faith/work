...
class MySpider(spider.Spider):
    def __init__(self, cmd_args=None):
        ...
        self.siteName = "岑溪检察"
        self.site_domain = 'cxsjcy.gov.cn'
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "01"
        self.start_urls = [
            'http://www.cxsjcy.gov.cn/index.php?m=content&c=index&a=lists&catid=10'
        ]
    ...
    def parse(self, response):
        ...
        if response is not None:
            ...
            urls = data.xpathall('''//div[@class="main"]/div[@class="col-left"]/ul//a''')
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        channel = data.xpath('''//div[@class="crumbs"]/a[2]/text()''').text()
        title = data.xpath('''//h1/text()''').text().strip()
        source = data.xpath('''//div[@id="Article"]/h1/span''').text().strip()
        source = re.findall('来源：(.*?)评论',source)[0]
        visit_count = data.xpath('''//*[@id="hits"]''').text().strip()
        reply_count = data.xpath('''//a[@id="comment"]/text()''').regex("(\d+)").int()
        ctime = data.xpath('''//div[@id="Article"]/h1/span''').regex('\d+-\d+-\d+ \d+:\d+:\d+').datetime()
        if ctime < self.c_time:
            return
        content = data.xpath('''//div[@class="content"]''').text().strip()
        # imgs = data.xpathall('''((//div[@class="t_fsz"])[1])//img/@src''')
        imgs = []
        if len(imgs) > 0:
            for i in imgs:
                img_url = i.text()
...
if __name__ == '__main__':
    ...
    url = 'http://www.cxsjcy.gov.cn/index.php?m=content&c=index&a=lists&catid=10'
    resp = spider.download(url)
