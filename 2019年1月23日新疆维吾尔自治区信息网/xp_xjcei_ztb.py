...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "新疆维吾尔自治区信息网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://www.xj.cei.gov.cn"]
        self.encoding = 'utf-8'
        self.site_domain = 'xj.cei.gov.cn'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse(self, response, url):
        ...
        #     return ([], None, None)
        url_list = [
            'http://www.xj.cei.gov.cn/zhaotoubiao.jsp?urltype=tree.TreeTempUrl&wbtreeid=10856',
            # 招标公告

            'http://www.xj.cei.gov.cn/zhaotoubiao.jsp?urltype=tree.TreeTempUrl&wbtreeid=10857',
            # 中标公告
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        li_content = data.xpathall('''//table[@class="winstyle19964"]//tr[@height="20"]''')
        ...
        for item in li_content:
            ...
            date = item.xpath('''//span[@class="timestyle19964"]/text()''').text ().strip ()
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
            ...
            link = urljoin("http://www.xj.cei.gov.cn",link)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:xjcei", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:xjcei", url)
            return False
