...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '江苏省互联网协会'
        # 网站一级域名
        self.site_domain = 'jsia.org.cn'

        self.start_urls = [

            # 列表页链接
            'http://www.jsia.org.cn/ArticleList/952478807242.html',
            'http://www.jsia.org.cn/ArticleList/107936705820.html',
            'http://www.jsia.org.cn/ArticleList/910982850419.html',
            'http://www.jsia.org.cn/ArticleList/885922958307.html',
            'http://www.jsia.org.cn/ArticleList/308378038508.html',
            'http://www.jsia.org.cn/ArticleList/789275722778.html'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="right_con"]//li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.jsia.org.cn", post_url[3:])
                # 模块内解析详情页时间，一般以//span、//div、//p等结尾，如没有则删除这三行
                # 例：ctime = item.xpath('''//div[@class="tail"]//span[1]''').datetime()
                ctime = item.xpath('''//span[@class="c333"]''').datetime()
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//li[@id="content"]''')
        ...
        pic_urls_list = data.xpathall('''//li[@id="content"]//img/@src''')
        if pic_urls_list:
