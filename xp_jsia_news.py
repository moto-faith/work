...
class MySpider(spider.Spider):
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        channel = data.xpath('''//span[@class="more c66"]/a[3]''').text()
        title = data.xpath('''//h1[@class="news_title"]''').text().strip()
        source = data.xpath('''//div[@class="news"]/ul/li[1]''').text().strip()
        ...
        if ctime < self.c_time:
            return
        content = data.xpath('''//li[@id="content"]''').text().strip()
        imgs = data.xpathall('''//li[@id="content"]//img/@src''')
        # imgs = []
        if len(imgs) > 0:
            for i in imgs:
                img_url = i.text()
...
if __name__ == '__main__':
    ...
    url = 'http://www.jsia.org.cn/ArticleDetail/518123206497.html'
    resp = spider.download(url)
