...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '台州档案信息网'
        # 网站一级域名
        self.site_domain = 'tzdaj.com.cn'

        self.start_urls = [
            # 列表页链接
            'http://tzdaj.com.cn/col/col17/index.html',#便民资讯
            'http://tzdaj.com.cn/col/col15/index.html',#台州要闻
            'http://tzdaj.com.cn/col/col2847/index.html',# 部门动态
            'http://tzdaj.com.cn/col/col2848/index.html',#县市报道
            'http://tzdaj.com.cn/col/col36/index.html',# 	民意征集
            'http://tzdaj.com.cn/col/col14/index.html',#通知公告
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            for item in re.findall("href='(/art.*?)'",data.response.text):  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                # if not post_url:
                #     continue
                post_url = urljoin('http://tzdaj.com.cn',item)
    ...
    def get_detail_page_info(self, data):
        ...
        channel =data.xpath('''//meta[@name="channel"]/@content''').text().strip()
        ...
        retweeted_source =data.xpath('''//meta[@name="source"]/@content''').text().strip() or self.siteName
