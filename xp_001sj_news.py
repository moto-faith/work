...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '松江第一招聘网'
        # 网站一级域名
        self.site_domain = '001sj.net'

        self.start_urls = [
            # 列表页链接
            'http://www.001sj.net/news/news_1.html', #松江资讯
            'http://www.001sj.net/news/news_2.html',  # 松江楼市
            'http://www.001sj.net/news/news_6.html',  # 松江网友爆料
            'http://www.001sj.net/news/news_4.html',  # 松江人才
            'http://www.001sj.net/news/news_3.html', # 松江旅游
            'http://www.001sj.net/news/news_26.html' # 松江企业
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="newsclasslist"]//li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = "http://www.001sj.net/news/"+post_url
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="title"]//h1//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="title"]//font[@color="#009900"]''').regex('\d+-\d+-\d+ \d+:\d+\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//*[@id="newsinfo"]/dl/dt/a[2]//text()''').text().strip()
        ...
        retweeted_source = data.xpath('''//div[@class="title"]/a[1]//text()''').text().strip() or  self.siteName
