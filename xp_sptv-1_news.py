...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '遂平广视在线'
        # 网站一级域名
        self.site_domain = 'sptv-1.com'

        self.start_urls = [
            # 列表页链接
            'http://www.sptv-1.com/xinwenzixun/suipingxinwen',
            'http://www.sptv-1.com/xinwenzixun/benshengxinwen',
            'http://www.sptv-1.com/xinwenzixun/guoneixinwen',
            'http://www.sptv-1.com/xinwenzixun/guojixinwen',
            'http://www.sptv-1.com/xinwenzixun/tushuosuiping',
            'http://www.sptv-1.com/xinwenzixun/redianxinwen',
            'http://www.sptv-1.com/xinwenzixun/lingdaogongtai',
            'http://www.sptv-1.com/xinwenzixun/shehuiwanxiang'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'UTF-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="row listing listing-icon"]/div''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                post_url = item.xpath('''//a[@class="image-link"]/@href''').text().strip()
                if not post_url:
                    continue
                # post_url = urljoin("http://www.fm086.com", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//h1[@class="post-title"]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//time[@class="value-datetime"]/@datetime''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//span[@class="cats"]/a[2]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@class="post-meta"]//a[@rel="author"]//text()''')
        source = data.xpath('''//div[@class="post-meta"]//a[@rel="author"]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="post-container cf"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="post-container cf"]//img/@src''')
        if pic_urls_list:
