...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '莆田市人民政府门户网站'
        # 网站一级域名
        self.site_domain = 'putian.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.putian.gov.cn/zwgk/ptdt/ptyw/',#莆田要闻
            'http://www.putian.gov.cn/zwgk/ptdt/ldhd/',#领导活动
            'http://www.putian.gov.cn/zwgk/ptdt/xqdt/',# 县区动态
            'http://www.putian.gov.cn/zwgk/ptdt/bmdt/',#部门动态
            'http://www.putian.gov.cn/zwgk/ptdt/zxhy/',  #最新会议
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@style="display: block;"]//li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.putian.gov.cn',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        channel =data.xpath('''//meta[@name="ColumnName"]/@content''').text().strip()
