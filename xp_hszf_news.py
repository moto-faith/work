...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '横山区人民政府门户网站'
        # 网站一级域名
        self.site_domain = 'hszf.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.hszf.gov.cn/zhengwuyaowen/index.jhtml',#政务要闻
            'http://www.hszf.gov.cn/tongzhigonggao/index.jhtml',# 通知公告
            'http://www.hszf.gov.cn/jiezhenyaowen/index.jhtml',#  街镇动态
            'http://www.hszf.gov.cn/bumendongtai/index.jhtml',# 部门动态
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="new-list-nameul"]//a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                # post_url = urljoin('http://www.qishan.gov.cn',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="detail-name"]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="detail-time"]/text()[1]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//div[@class="fl-l new-site"]/a[2]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source =data.xpath('''//div[@class="detail-time"]/text()[2]''').text().strip() or self.siteName

        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source =data.xpath('''//div[@class="detail-time"]/label//text()''').text().strip() or  self.siteName
        ...
        content1 = data.xpathall('''//div[@class="detail-content"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="detail-content"]//img/@src''')
        if pic_urls_list:
