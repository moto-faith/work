...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '客都网'
        # 网站一级域名
        self.site_domain = 'mz186.com'

        self.start_urls = [
            # 列表页链接
            'http://www.mz186.com/xinwen/shizheng/', # 时政资讯
            'http://www.mz186.com/xinwen/guonei/',  # 国内资讯
            'http://www.mz186.com/xinwen/guoji/',  # 国际资讯
            'http://www.mz186.com/xinwen/fangtan/', #访谈资讯
            'http://www.mz186.com/xinwen/zhengwu/',#政务资讯
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="list"]/ul//h2''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.mz186.com",post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//p[@class="publish_time"]''').regex('\d+-\d+-\d+ \d+:\d+:\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
