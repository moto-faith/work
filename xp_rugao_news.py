...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '江苏如皋政府门户网站'
        # 网站一级域名
        self.site_domain = 'rugao.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.rugao.gov.cn/rgsrmzf/xwzx/xwzx.html', #新闻中心
            'http://www.rugao.gov.cn/rgsrmzf/jrtt/jrtt.html', #如皋头条
            'http://www.rugao.gov.cn/rgsrmzf/ywkd/ywkd.html', #要闻快递
            'http://www.rugao.gov.cn/rgsrmzf/bddt/bddt.html', #本地动态
            'http://www.rugao.gov.cn/rgsrmzf/tpxw/tpxw.html'  # 图片新闻
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@id="initData"]//li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.rugao.gov.cn", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source =data.xpath('''//div[@class="stitle"]/span[1]//text()''').text().strip() or self.siteName
