...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '南昌大学科学技术学院'
        # 网站一级域名
        self.site_domain = 'ndkj.com.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=01',#最新要闻
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=008',#教学科研
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=0012',#校园评论
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=003',#动态新闻
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=009',  #社会广角
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=0010',  #合作交流
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=004',  #人物专访
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=0013',  #党建思政
            'http://www.ndkj.com.cn/News/mainNewsList.aspx?boardid=006',  #新芽怒绽
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//td[@class="btmline"]/a''')
            # self.channel = data.xpath('''//div[@class="title"]/span''').text().strip()
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = "http://www.ndkj.com.cn/News/"+post_url
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//td[@class="biaoti"]/span//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//span[@id="mainRecordDetail1_labPubDate"]/text()''').regex('\d+/\d+/\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        # channel = data.xpath('''//td[@height="33"]/a[3]//text()''').text().strip()
        channel = data.xpath('''//span[@id="mainRecordDetail1_lab_Hint"]/a[3]//text()''').text().strip()
        # 详情页解析作者，一般以//text()结尾
        ...
        content1 = data.xpathall('''//td[@height="400"]''')
        ...
        pic_urls_list = data.xpathall('''//td[@height="400"]//img/@src''')
        if pic_urls_list:
