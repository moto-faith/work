...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '黑龙江会展网'
        # 网站一级域名
        self.site_domain = 'china-russiaexpo.org.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.china-russiaexpo.org.cn/ltd/hqdt.php?key19=ichtfpicnews',
            'http://www.china-russiaexpo.org.cn/ltd/hqdt.php?key19=20th',
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                if 'www.china-russiaexpo.org.cn/php/news' not in post_url:
                    continue
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@align="center"]//strong//text()''').text().strip()
        if not title:
            return
        ...
        ctime = data.xpath('''//div[@align="center"]/font[2]//text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//ul[@class="menu"]/li[6]/a''').text().strip()
        ...
        retweeted_source = data.xpath('''//div[@align="center"]/font[3]/text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//span[@class="STYLE444"]/p''')
        ...
        pic_urls_list = data.xpathall('''//span[@class="STYLE444"]//img/@src''')
        if pic_urls_list:
