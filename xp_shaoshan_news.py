...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '韶山政府门户网'
        # 网站一级域名
        self.site_domain = 'shaoshan.gov.cn'

        self.start_urls = [

            # 列表页链接
            'http://www.shaoshan.gov.cn/Category_1/Index.aspx',
            'http://www.shaoshan.gov.cn/Category_623/Index.aspx',
            'http://www.shaoshan.gov.cn/Category_3519/Index.aspx'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="newsList"]/li''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.shaoshan.gov.cn", post_url)
                # 模块内解析详情页时间，一般以//span、//div、//p等结尾，如没有则删除这三行
                # 例：ctime = item.xpath('''//div[@class="tail"]//span[1]''').datetime()
                ctime = item.xpath('''//span[@class="date"]''').datetime()
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@class="conTxt"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="conTxt"]//img/@src''')
        if pic_urls_list:
