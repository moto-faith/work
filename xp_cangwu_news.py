...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '苍梧县人民政府门户网站'
        # 网站一级域名
        self.site_domain = 'cangwu.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.cangwu.gov.cn/type.php?typeID=1202',# 苍梧要闻
            'http://www.cangwu.gov.cn/type.php?typeID=1203',#通知公告
            'http://www.cangwu.gov.cn/type.php?typeID=1204',#部门动态
            'http://www.cangwu.gov.cn/type.php?typeID=1205',  #乡镇动态
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            # loops = data.xpathall('''//div[@class="fltlftM10"]//a''')
            # self.channel = data.xpath('''//div[@class="main_title"]/text()''').text().strip()
            for item in re.findall('href="(info.php.*?)"',data.response.text):  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                # if not post_url:
                #     continue
                post_url = urljoin('http://www.cangwu.gov.cn', item)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//meta[@name='ArticleTitle']/@content''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//meta[@name='PubDate']/@content''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//meta[@name='ColumnType']/@content''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source =data.xpath('''//meta[@name='Author']/@content''').text().strip() or self.siteName

        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source =data.xpath('''//meta[@name='ContentSource']/@content''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="fltlftM"]/p''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="fltlftM"]/p//img/@src''')
        if pic_urls_list:
