...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '濮阳房产网'
        # 网站一级域名
        self.site_domain = 'pyfc.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.pyfc.cn/class.asp?class=4', #房产动态
            'http://www.pyfc.cn/class.asp?class=56',  # 楼盘动态
            'http://www.pyfc.cn/class.asp?class=107',#主编荐房
            'http://www.pyfc.cn/class.asp?class=108',#人物访谈
            'http://www.pyfc.cn/class.asp?class=109',#看房回顾
            'http://www.pyfc.cn/class.asp?class=97',# 楼盘公示
            'http://www.pyfc.cn/class.asp?class=59',  #购房指南
            'http://www.pyfc.cn/class.asp?class=61',#政策法规
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//li[@class="item nBor"]''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                post_url = item.xpath('''//h4//a//@href''').text().strip()
                if not post_url:
                    continue
                post_url = urljoin('http://www.pyfc.cn' , post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source = data.xpath('''//div[@class="info-source"]/span[2]/text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@id="newscontent"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="newscontent"]//img/@src''')
        if pic_urls_list:
