...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '中共广州市委党校'
        # 网站一级域名
        self.site_domain = 'gzswdx.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.gzswdx.gov.cn/xwzx/xyxw/', #校院新闻
            'http://www.gzswdx.gov.cn/xwzx/tpxw/',  # 图片新闻
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//*[@id="zfxxdiv"]/table[1]//tr/td[2]''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                if url == 'http://www.gzswdx.gov.cn/xwzx/tpxw/':
                    post_url = "http://www.gzswdx.gov.cn/xwzx/tpxw"+post_url[1:]
                else:
                    post_url = "http://www.gzswdx.gov.cn/xwzx/xyxw" + post_url[1:]
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//td[@align="middle"]//text()''').text().strip() or data.xpath('''//td[@align="center"]//text()''').text().strip()
        ...
        content1 = data.xpathall('''//td[@id="fontzoom"]''') or data.xpathall('''//div[@class="Custom_UnionStyle"]''')
        ...
        pic_urls_list = data.xpathall('''//td[@id="fontzoom"]//img/@src''')
        if pic_urls_list:
