...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '舟山卫生监督网'
        # 网站一级域名
        self.site_domain = 'zswsjd.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.zswsjd.gov.cn/list.asp?classid=3',#公告通知
            'http://www.zswsjd.gov.cn/list.asp?classid=4',#监督时讯
            'http://www.zswsjd.gov.cn/list.asp?classid=19',#定海区
            'http://www.zswsjd.gov.cn/list.asp?classid=20',#普陀区
            'http://www.zswsjd.gov.cn/list.asp?classid=21',# 岱山县
            'http://www.zswsjd.gov.cn/list.asp?classid=22',#嵊泗县
            'http://www.zswsjd.gov.cn/list.asp?classid=8',#健康教育
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="listul"]/li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.zswsjd.gov.cn',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="hd"]/h1//text()''').text().strip()
        ...
        channel = data.xpath('''//div[@class="location"]/a[2]//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source =data.xpath('''//div[@class="titBar"]/span[1]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@id="pagetext"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="pagetext"]//img/@src''')
        if pic_urls_list:
