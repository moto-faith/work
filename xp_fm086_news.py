...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '中国耐材之窗网'
        # 网站一级域名
        self.site_domain = 'fm086.com'

        self.start_urls = [
            # 列表页链接
            'http://www.fm086.com/Article/ArticleList?typeid=8',
            'http://www.fm086.com/Article/ArticleList?typeid=19',
            'http://www.fm086.com/Article/ArticleList?typeid=7',
            'http://www.fm086.com/Article/ArticleList?typeid=2',
            'http://www.fm086.com/patent/patentlist?Type=5',
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="y_price_table_ul"]/li''')
            self.channel = data.xpath('''//div[@class="y_txt_left_tit"]/span''').text().strip()
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="y_price_table"]/span//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="y_txt_tit2"]/span//text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        content1 = data.xpathall('''//div[@id="ArticleContent"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="ArticleContent"]//img/@src''')
        if pic_urls_list:
