...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '崇左广播电视网'
        # 网站一级域名
        self.site_domain = 'gxcztv.cn'

        self.start_urls = [

            # 列表页链接
            'http://www.gxcztv.cn/sy/tj/jrrd/index.shtml'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="content_first"]//li''')
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//span[@class="detail_name"]''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//span[@class="detail_date"]//text()''').replace('年','').replace('月','').replace('日','').datetime()
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="now_pos "]/a[4]//text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@id="editWrap"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="editWrap"]//img/@src''')
        if pic_urls_list:
