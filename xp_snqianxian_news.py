...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '乾县人民政府'
        # 网站一级域名
        self.site_domain = 'snqianxian.gov.cn	'

        self.start_urls = [
            # 列表页链接
            'http://www.snqianxian.gov.cn/html/xwzx/tzgg/index.html',#通知公告
            'http://www.snqianxian.gov.cn/html/xwzx/jryw/index.html',#今日要闻
            'http://www.snqianxian.gov.cn/html/xwzx/bmdt/index.html',# 部门动态
            'http://www.snqianxian.gov.cn/html/xwzx/zcfc/index.html',#镇村风采
            'http://www.snqianxian.gov.cn/html/xwzx/mtjj/index.html',  #媒体聚焦
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="ny_right_conList"]//li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.snqianxian.gov.cn',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@id="info_content"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="info_content"]//img/@src''')
        if pic_urls_list:
