...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '西游车展网'
        # 网站一级域名
        self.site_domain = 'xcabc.com'

        self.start_urls = [
            # 列表页链接
            'http://cz.xcabc.com/News/CarNewsList.aspx?typeid=68',# 人物专访
            'http://cz.xcabc.com/News/CarNewsList.aspx?typeid=62',#车市观察
            'http://cz.xcabc.com/News/CarNewsList.aspx?typeid=61',#车展观察
            'http://cz.xcabc.com/News/CarNewsList.aspx?typeid=63',#车企动态
            'http://cz.xcabc.com/News/CarNewsList.aspx?typeid=64',#销售数据
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//p[@class="til"]/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                # post_url = urljoin('http://www.mizhi.gov.cn', item)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@class="news-content"]/p''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="news-content"]/p//img/@src''')
        if pic_urls_list:
