...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '甘肃零距离'
        # 网站一级域名
        self.site_domain = 'gs090.com'

        self.start_urls = [
            # 列表页链接
            'http://www.gs090.com/news/gansu/index.html',#社会
            'http://www.gs090.com/news/wenhua/index.html',# 文化
            'http://www.gs090.com/news/lvyou/index.html',#  旅游
            'http://www.gs090.com/news/yule/index.html',#娱乐
            'http://www.gs090.com/news/fangchan/index.html',#房产
            'http://www.gs090.com/news/tushuo/index.html',#图说
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                # post_url = urljoin('http://www.164580.com/article',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@id="Article"]/h1[1]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="article_info"]/text()[1]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//span[@class="cat-name-list"]/a[2]/text()''').text().strip()
        ...
        retweeted_source = data.xpath('''//div[@class="article_info"]/a/text()''').text().strip() or self.siteName
