...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '万鹤来旅游网'
        # 网站一级域名
        self.site_domain = 'hebily.net'

        self.start_urls = [
            # 列表页链接
            'http://www.hebily.net/news/social/', #社会
            'http://www.hebily.net/news/china/',  # 国内
            'http://www.hebily.net/news/world/',  # 国际
            'http://www.hebily.net/news/zt/',  # 专题
            'http://www.hebily.net/news/org/', # 原创
            'http://www.hebily.net/news/yidu/', #军事
            'http://www.hebily.net/news/jy/'#教育
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="news_list"]//li''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//span[@id="pubtime_baidu"]/text()''').regex('\d+-\d+-\d+ \d+:\d+:\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        content1 = data.xpathall('''//div[@class="main_article"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="main_article"]//img/@src''')
        if pic_urls_list:
