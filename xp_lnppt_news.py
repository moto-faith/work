...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '辽宁品牌网'
        # 网站一级域名
        self.site_domain = 'lnppt.com'

        self.start_urls = [
            # 列表页链接
            'http://www.lnppt.com/newsclass.asp?id=100',
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//td[@width="711"]//td[1]''')
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                if not 'news' in post_url:
                    continue
                post_url = urljoin("http://www.lnppt.com/", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//span[@class="STYLE1"]''')
        ...
        pic_urls_list = data.xpathall('''//span[@class="STYLE1"]//img/@src''')
        if pic_urls_list:
