...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '聊城传媒网'
        # 网站一级域名
        self.site_domain = '365chm.com'

        self.start_urls = [
            # 列表页链接
            'http://www.365chm.com/fo/',# 禅林
            'http://www.365chm.com/guoxue/society/',#国学
            'http://www.365chm.com/baby/',#亲子
            'http://www.365chm.com/food/',  #美食
            'http://www.365chm.com/home/',  #家居
            'http://www.365chm.com/law/',#商界
            'http://www.365chm.com/lcsh/',  #聊城社会
            'http://www.365chm.com/mtklc/',  #聊城资讯
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        ...
        self.page_url = {}
        self.max_interval = datetime.timedelta(days=30)
        # self.dedup_uri=None
        self.channel = ''
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@id="index-m-list"]//h1/a''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//span[@class="article-time"]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@id="navList"]/a[2]/text()''').text().strip()
        ...
        retweeted_source =data.xpath('''//span[@bosszone="jgname"]/a/text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@bosszone="content"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@bosszone="content"]//img/@src''')
        if pic_urls_list:
