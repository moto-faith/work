...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '中国人口信息网'
        # 网站一级域名
        self.site_domain = 'cpirc.org.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.cpirc.org.cn/xinwen/shizheng/', #时政资讯
            'http://www.cpirc.org.cn/xinwen/guonei/',  # 国内资讯
            'http://www.cpirc.org.cn/xinwen/guoji/',#国际资讯
            'http://www.cpirc.org.cn/xinwen/fangtan/',#访谈资讯
            'http://www.cpirc.org.cn/xinwen/zhengwu/',  #政务资讯
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="imgr"]''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                post_url = item.xpath('''//h2//a//@href''').text().strip()
                if not post_url:
                    continue
                post_url = urljoin('http://www.cpirc.org.cn' , post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="nrHeader"]/h1//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//p[@class="publish_time"]//text()''').regex('\d+-\d+-\d+').datetime()
        if ctime < self.c_time:
           return None
        ...
        retweeted_source =data.xpath('''//p[@class="resource"]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="textBody"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="textBody"]//img/@src''')
        if pic_urls_list:
