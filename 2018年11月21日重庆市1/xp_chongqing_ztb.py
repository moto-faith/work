...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "重庆市公共资源交易网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["https://www.cqggzy.com"]
        self.encoding = 'utf-8'
        self.site_domain = 'cqggzy.com'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse(self, response, url):
        ...
        #     return ([], None, None)
        url_list = [
            'https://www.cqggzy.com/web/services/PortalsWebservice/getCQInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002001&infoD=&title=&infoC=&_=1542767866069',
            # 工程建设招标公告

            'https://www.cqggzy.com/web/services/PortalsWebservice/getInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002002&title=&infoC=&_=1542768311221',
            # 工程建设招标澄清

            'https://www.cqggzy.com/web/services/PortalsWebservice/getInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002004&title=&infoC=&_=1542768346294',
            # 工程建设中标公告
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
        if from_tag_url == "https://www.cqggzy.com/web/services/PortalsWebservice/getCQInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002001&infoD=&title=&infoC=&_=1542767866069":
            tag = "招标公告"
        elif from_tag_url == "https://www.cqggzy.com/web/services/PortalsWebservice/getInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002002&title=&infoC=&_=1542768311221":
            tag = "变更通知"
        elif from_tag_url == "https://www.cqggzy.com/web/services/PortalsWebservice/getInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002004&title=&infoC=&_=1542768346294":
            tag = "中标公告"
        else:
            tag = "招标公告"

        titles = re.findall('title\\\\":\\\\"(.*?)\\\\"', unicode_html_body)
        dates = re.findall('infodate\\\\":\\\\"(.*?)\\\\"', unicode_html_body)
        links = re.findall('infourl\\\\":\\\\"(.*?)\\\\"',unicode_html_body)

        for title,date,link in zip(titles,dates,links):
            date = str(date).replace("-", "")
            link = 'https://www.cqggzy.com'+str(link).replace("\\","")
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:cqggzy", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:cqggzy", url)
            return False
...
if __name__ == '__main__':
    ...
    # url = "https://www.cqggzy.com/web/services/PortalsWebservice/getInfoList?response=application/json&pageIndex=1&pageSize=18&siteguid=d7878853-1c74-4913-ab15-1d72b70ff5e7&categorynum=014002004&title=&infoC=&_=1542768346294"
