...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "青海省招标投标网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://qhzbtb.qhwszwdt.gov.cn"]
        self.encoding = 'gb2312'
        self.site_domain = 'qhzbtb.qhwszwdt.gov.cn'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse(self, response, url):
        ...
        url_list = [
            ...
            'http://qhzbtb.qhwszwdt.gov.cn/qhweb/jyxx/005004/005004001/MoreInfo.aspx?CategoryNum=005004001',
            # 招标公告
            ...
            'http://qhzbtb.qhwszwdt.gov.cn/qhweb/jyxx/005004/005004003/MoreInfo.aspx?CategoryNum=005004003',
            # 中标公告
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        # titles = re.findall(
        #     'td align="left" valign="middle" style="border-style:None;".*?href=".*?" target="_blank" title="(.*?)"',
        #     unicode_html_body, re.S)
        # links = re.findall(
        #     'td align="left" valign="middle" style="border-style:None;".*?href="(.*?)" target="_blank" title=".*?"',
        #     unicode_html_body, re.S)
        # dates = re.findall('<td align="center" valign="middle" style="border-style:None;width:80px;">(.*?)</td>',
        #                    unicode_html_body, re.S)
        li_content = data.xpathall('''//table[@id="MoreInfoList1_DataGrid1"]//tr''')
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
            ...
            link = "http://qhzbtb.qhwszwdt.gov.cn"+link
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:qhwszwdt", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:qhwszwdt", url)
            return False
...
if __name__ == '__main__':
    ...
    url = "http://qhzbtb.qhwszwdt.gov.cn/qhweb/jyxx/005001/005001001/MoreInfo.aspx?CategoryNum=005001001"
