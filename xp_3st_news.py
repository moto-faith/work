...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '四川自考信息网'
        # 网站一级域名
        self.site_domain = '3st.org'

        self.start_urls = [
            # 列表页链接
            'http://www.3st.org/more.asp?bigclassname=%D5%D0%C9%FA%D7%CA%D1%B6',#招生资讯
            'http://www.3st.org/more.asp?bigclassname=%CD%F8%BD%CC',#网教
            'http://www.3st.org/more.asp?bigclassname=%D6%B0%D2%B5%D6%B8%B5%BC',#职业指导
            'http://www.3st.org/more.asp?bigclassname=%B6%CC%C6%DA%C5%E0%D1%B5',#短期培训
            'http://www.3st.org/more.asp?bigclassname=%D5%FE%B2%DF%B7%A8%B9%E6',# 政策法规
            'http://www.3st.org/more.asp?bigclassname=%BD%CC%D1%A7%BD%CC%D1%D0',#教学教研
            'http://www.3st.org/more.asp?bigclassname=%D7%A8%B1%BE%D1%A7%C0%FA',#专本学历
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//td[@height="30"]/a''')
            # self.channel = data.xpath('''//div[@class="titlef"]/h2/text()''').text().strip()
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.3st.org',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//table[@width="100%"]//tr[3]''')
        ...
        pic_urls_list = data.xpathall('''//table[@width="100%"]//tr[3]//img/@src''')
        if pic_urls_list:
