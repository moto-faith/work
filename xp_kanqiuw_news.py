...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = 'JRs看球网'
        # 网站一级域名
        self.site_domain = 'kanqiuw.com'

        self.start_urls = [
            # 列表页链接
            'http://www.kanqiuw.com/article/yzbqz',# 亚洲杯前瞻
            'http://www.kanqiuw.com/article/yzbzr',#亚洲杯阵容
            'http://www.kanqiuw.com/article/yzbzb',#亚洲杯战报
            'http://www.kanqiuw.com/article/yzbnews',  #亚洲杯新闻
            'http://www.kanqiuw.com/article/zuqiutj',  #足球推荐
            'http://www.kanqiuw.com/article/nbatj',#NBA推荐
            'http://www.kanqiuw.com/article/soccernews',  #足球新闻
            'http://www.kanqiuw.com/article/nbanews',  #NBA新闻
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//li[@class="artEv"]/a[2]''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="title"]/time''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//ul[@class="nav-list"]/li[10]/a/text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@class="articles_text"]/p''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="articles_text"]/p//img/@src''')
        if pic_urls_list:
