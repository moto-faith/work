...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '云南省国土资源厅'
        # 网站一级域名
        self.site_domain = 'yndlr.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.yndlr.gov.cn/html/zfjg/index.html',# 要闻播报
            'http://www.yndlr.gov.cn/html/zwdt/index.html',#工作动态
            'http://www.yndlr.gov.cn/html/csgh/index.html',#州县动态
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="common-list-container"]/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.yndlr.gov.cn", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="nei_shi"]/span[2]//text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="website"]/a[2]//text()''').text().strip()
        ...
        retweeted_source =data.xpath('''//div[@class="nei_shi"]/span[1]//text()''').text().strip() or  self.siteName
        ...
        content1 = data.xpathall('''//div[@class="nei_p"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="nei_p"]//img/@src''')
        if pic_urls_list:
