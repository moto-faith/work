...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '张店区政府公众信息网'
        # 网站一级域名
        self.site_domain = 'zhangdian.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://zhangdian.gov.cn/col/col6078/index.html',# 今日关注
            'http://zhangdian.gov.cn/col/col6079/index.html',#部门动态
            'http://zhangdian.gov.cn/col/col6080/index.html',#街道（乡镇）动态
            'http://zhangdian.gov.cn/col/col6081/index.html',#通知公告
            'http://zhangdian.gov.cn/col/col6083/index.html',#图片新闻
            'http://zhangdian.gov.cn/col/col7060/index.html',#市政府信息
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            # loops = data.xpathall('''//div[@class="default_pgContainer"]/li/a''')
            # self.channel = data.xpath('''//div[@class="page-title"]//text()''').text().strip()
            for item in re.findall("</i><a href='(.*?)'",data.response.text):  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                # if not post_url:
                #     continue
                post_url = urljoin('http://zhangdian.gov.cn', item)
