...
class MySpider(spider.Spider):
    ...
    def parse(self, response):
        ...
        if response is not None:
            ...
            urls = data.xpathall('''//a[@class="linklj"]''')
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        channel = data.xpath('''//td[3]//a[3]''').text()
        title = data.xpath('''//strong[@class="titlezt"]''').text().strip()
        # source = data.xpath('''//div[@class="de_title"]/span[1]/a''').text().strip()
        source = ''
        # visit_count = data.xpath('''//td[@class="tex"]//text()''').regex('访问：\d+人')
        visit_count = 0
        reply_count = 0
        # reply_count = data.xpath('''//a[@id="comment"]/text()''').regex("(\d+)").int()
        ctime = data.xpath('''//td[@class="tex"]//text()''').regex('\d+年\d+月\d+日').datetime()
        if ctime < self.c_time:
            return
        content = data.xpath('''//td[@class="showzt"]''').text().strip()
...
if __name__ == '__main__':
    ...
    url = 'http://www.hzzjj.gov.cn/html/show.asp?id=7582'
    resp = spider.download(url)
