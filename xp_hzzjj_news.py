...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '湖州市质量技术监督局'
        # 网站一级域名
        self.site_domain = 'hzzjj.gov.cn'

        self.start_urls = [

            # 列表页链接
            'http://www.hzzjj.gov.cn/html/zjj.asp?classid=3',
            'http://www.hzzjj.gov.cn/html/zjj.asp?classid=3&Nclassid=90',
            'http://www.hzzjj.gov.cn/html/zjj.asp?classid=3&Nclassid=12',
            'http://www.hzzjj.gov.cn/html/zjj.asp?classid=3&Nclassid=13'
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//a[@class="linklj"]''')
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//strong[@class="titlezt"]''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//td[@class="tex"]//text()''').replace('年','').replace('月','').replace('日','').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        # channel = data.xpath('''//td[3]//a[3]//text()''').text().strip()
        channel = data.xpath('''(//a[@class="nav"])[1]''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source = data.xpath('''//div[@class="about_left1w"]/a[3]//text()''').text().replace('来源：',
                                                                                      '').strip() or self.siteName

        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source = data.xpath('''//div[@class="about_left1w"]/a[3]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//td[@class="showzt"]''')
        ...
        pic_urls_list = data.xpathall('''//td[@class="showzt"]//img/@src''')
        if pic_urls_list:
