...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '安徽信息网'
        # 网站一级域名
        self.site_domain = 'in.ah.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.in.ah.cn/list.jsp?classID=10000996',# 单位概况
            'http://www.in.ah.cn/list.jsp?classID=10001000',#中心动态
            'http://www.in.ah.cn/list.jsp?classID=10001004',#预决算公开
            'http://www.in.ah.cn/list.jsp?classID=10001008',  #党建工作
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="main_list"]/li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.in.ah.cn', post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="main_cont"]/h1//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="main_cont"]/h3/span[1]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        source = data.xpath('''//div[@class="main_cont"]/h3/span[3]//text()''').text().strip() or  self.siteName

        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source =data.xpath('''//div[@class="main_cont"]/h3/span[2]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="main_cont_txt"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="main_cont_txt"]//img/@src''')
        if pic_urls_list:
