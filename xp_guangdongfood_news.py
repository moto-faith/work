...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '广东食品网'
        # 网站一级域名
        self.site_domain = 'guangdongfood.net'

        self.start_urls = [
            # 列表页链接
            'http://guangdongfood.net/ShowNews/NewsList/?cname=%E7%83%AD%E7%82%B9%E6%96%B0%E9%97%BB&tname=%E9%A3%9F%E5%93%81%E8%B5%84%E8%AE%AF',# 热点新闻
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="ModnewConOne"]/li''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://guangdongfood.net", post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source = data.xpath('''//span[@class="source"]//text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="textBox"]/p''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="textBox"]/p//img/@src''')
        if pic_urls_list:
