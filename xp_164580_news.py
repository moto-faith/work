...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '易紧通紧固件网'
        # 网站一级域名
        self.site_domain = '164580.com'

        self.start_urls = [
            # 列表页链接
            'http://www.164580.com/article/list_1.html',#业界信息
            'http://www.164580.com/article/list_4.html',# 原材料相关
            'http://www.164580.com/article/list_2.html',#  企业新闻
            'http://www.164580.com/article/list_3.html',#会议培训
            'http://www.164580.com/article/list_5.html',#标准与技术
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//a[@class="atitle"]''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="artInfo"]//font[1]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//div[@class="mton"]/text()''').text().strip()
        ...
        retweeted_source = data.xpath('''//div[@class="artInfo"]//font[2]/text()''').text().strip().replace('来源：','') or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="blkContainerSblkCon"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="blkContainerSblkCon"]//img/@src''')
        if pic_urls_list:
