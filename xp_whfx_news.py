...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '武汉房地产开发企业协会'
        # 网站一级域名
        self.site_domain = 'whfx.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.whfx.cn/index.php?m=content&c=index&a=lists&catid=87',#   协会动态
            'http://www.whfx.cn/index.php?m=content&c=index&a=lists&catid=85',#重要通知
            'http://www.whfx.cn/index.php?m=content&c=index&a=lists&catid=90',#人物访谈
            'http://www.whfx.cn/index.php?m=content&c=index&a=lists&catid=101',  #会员简介
            'http://www.whfx.cn/index.php?m=content&c=index&a=lists&catid=82',  #政策法规
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="list_tab"]/ul//a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                # post_url = urljoin('http://www.hncheshi.com', post_url)
