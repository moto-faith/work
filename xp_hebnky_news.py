...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '河北省农林科学院'
        # 网站一级域名
        self.site_domain = 'hebnky.com'

        self.start_urls = [
            # 列表页链接
            'http://www.hebnky.com/BKyuanneixinwenYaowen.aspx', #要闻
            'http://www.hebnky.com/BKyuanneixinwenYuansuodongtai.aspx',  # 院所动态
            'http://www.hebnky.com/BKyuanneixinwenMeitibaodao.aspx',#媒体报道
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//table[@rules="all"]//td[@align="left"]''')
            # self.channel = data.xpath('''//div[@id="zfxxtitlename"]/a[3]//text()''').text().strip()
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//span[@id="MainContent_DataList1_titleLabel_0"]//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//span[@id="MainContent_DataList1_timeLabel_0"]/text()''').regex('\d+-\d+-\d+ \d+:\d+:\d+').datetime()
        if ctime < self.c_time:
           return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//title//text()''').text().strip()

        # 详情页解析作者，一般以//text()结尾
        # source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        source = data.xpath('''//span[@id="MainContent_DataList1_zuozheLabel_0"]/text()''').text().strip() or self.siteName

        # 详情页解析来源，一般以//text()结尾，如没有，此字段=''
        # retweeted_source = data.xpath('''//div[@id="xl-headline"]//div[@class="left"]//text()''')
        retweeted_source = data.xpath('''//span[@id="MainContent_DataList1_laiyuanLabel_0"]/text()''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//span[@id="MainContent_DataList1_neirongLabel_0"]''')
        ...
        pic_urls_list = data.xpathall('''//span[@id="MainContent_DataList1_tupianLabel_0"]//tr[4]//img/@src''')
        if pic_urls_list:
