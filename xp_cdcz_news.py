...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '常德市财政局'
        # 网站一级域名
        self.site_domain = 'cdcz.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://czj.changde.gov.cn/col/col6496/index.html',# 资讯中心
            'http://czj.changde.gov.cn/col/col6504/index.html',#本市财经动态
            'http://czj.changde.gov.cn/col/col6505/index.html',#国内财经动态
            'http://czj.changde.gov.cn/col/col6503/index.html',  #公示公告
            'http://czj.changde.gov.cn/col/col33322/index.html',  #头条新闻
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            for item in re.findall("href='(/art.*?)'",data.response.text):  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                # if not post_url:
                #     continue
                post_url = urljoin('http://czj.changde.gov.cn', item)
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//*[@id="c"]//tr[2]/td/table//tr/td[1]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//td[2]/a[@class="bt_link"]//text()''').text().strip()
