...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '建设网'
        #网站一级域名
        self.site_domain = 'buildnet.cn'

        self.start_urls = [

            #列表页链接
            'http://www.buildnet.cn/NewsAndEvents/NewList.aspx?ID=31&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewList.aspx?ID=115&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewList.aspx?ID=112&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewList.aspx?ID=117&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewList.aspx?ID=114&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewList.aspx?ID=28&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewList.aspx?ID=62&GetType=Type',
            'http://news.buildnet.cn/NewsAndEvents/NewsListG.aspx',
            'http://www.buildnet.cn/NewsAndEvents/NewsListB.aspx'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="newstitle"]/a''')
            for item in loops: #这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.buildnet.cn",post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@id="detailcontent"]//p''')
        for item in content1:
            #此处填写需要排除的（不进行解析）元素，如有多个以‘|’分隔开，如没有，请将‘|？’删除，注意：‘//script’不要删除！
            #例：content_str = self.clear_special_xp(item,'''//script''')
            content_str = self.clear_special_xp(item,'''//script|//style|//ignore_js_op/div|//span[@style="display:none"]|//font[@class="jammer"]''')
