...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "内蒙古自治区政府采购网"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://www.nmgp.gov.cn"]
        self.encoding = 'utf-8'
        self.site_domain = 'nmgp.gov.cn'
        self.dedup_uri = None
        self.headers = {
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
        ...
        tb_id = re.findall('"ay_table_tag":"(.*?)"', unicode_html_body)
        p_id = re.findall('"wp_mark_id":"(.*?)"', unicode_html_body)

        for title,date,t, p in zip(titles,dates,tb_id, p_id):
            date = str(date)[:10].replace("-", "")
            link = "http://www.nmgp.gov.cn/ay_post/post.php?tb_id={}&p_id={}".format(t,p)
            title = str(title).decode('unicode-escape')
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:nmgp", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:nmgp", url)
            return False
...
if __name__ == '__main__':
    ...
    url = "http://www.nmgp.gov.cn/zfcgwslave/web/index.php?r=zfcgw%2Fanndata&type_name=3&byf_page=1&fun=cggg&_=1542766056744"
