...
class MySpider(spider.Spider):
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        if from_tag_url == "https://www.ciac.sh.cn/NetInterBidweb/GKTB/SgfbZbxx.aspx":
            tag = "招标公告"
            dates = data.xpathall('''//tr/td[3]/a''')
            ids = re.findall("openWindow\('(.*?)'", unicode_html_body, re.S)
            titles = re.findall('class="borderDealwith">(.*?)<', unicode_html_body, re.S)
            url = 'https://www.ciac.sh.cn/NetInterBidweb/GKTB/DefaultV2011.aspx?gkzbXh='
            for item, id, title in zip(dates, set(ids), titles):
                date = item.xpath('''//text()''').text().strip()
                link = url + id
            ...
            tag = "中标公告"
            dates = data.xpathall('''//tr[position()>=2]/td[3]''')
            titles = data.xpathall('''//td[@align="left"]/a''')
            link = from_tag_url
            for dateitem, titleitem in zip(dates, titles):
                date = dateitem.xpath('''//text()''').text().strip()
                title = titleitem.xpath('''//text()''').text().strip()
