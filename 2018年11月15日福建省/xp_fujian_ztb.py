...
class MySpider(spider.Spider):
    def __init__(self,
        ...
        # 网站名称
        self.siteName = "福建省公共资源交易中心"
        # 类别码，01新闻、02论坛、03博客、04微博 05平媒 06微信  07 视频、99搜索引擎
        self.info_flag = "99"

        # 入口地址列表
        # self.start_urls = ["http://www.bidcenter.com.cn/viplist-1.html"]
        self.start_urls = ["http://www.fjggzyjy.cn"]
        self.encoding = 'utf-8'
        self.site_domain = 'fjggzyjy.cn'
        self.dedup_uri = None
        self.headers = {
    ...
    def parse_detail_page(self, response=None, url=None):
        ...
        # titles = re.findall('class="btn btn-default article-list-single" href=".*?" title="(.*?)" target="_blank">',unicode_html_body,re.S)
        # links = re.findall('class="btn btn-default article-list-single" href="(.*?)"', unicode_html_body,re.S)
        # dates = re.findall('class="article-list-date">(.*?)</span>',unicode_html_body,re.S)
        # print dates
...
#         dates = data.xpathall('//span[@class="article-list-date"]/text()')
        if from_tag_url == "http://www.fjggzyjy.cn/news/category/10/":
            tag = "招标公告"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/12/":
            tag = "中标公告"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/46/":
            tag = "招标公告"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/49/":
            tag = "候选人公告"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/50/":
            tag = "中标公示"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/52/":
            tag = "招标公告"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/55/":
            tag = "候选人公告"
        elif from_tag_url == "http://www.fjggzyjy.cn/news/category/56/":
            tag = "中标公示"
        else:
            tag = "招标公告"

        li_content = data.xpathall('''//a[@class="btn btn-default article-list-single"]''')
        ...
        for item in li_content:
            ...
            link= item.xpath('''//@href''').text ().strip ()
            date = item.xpath('''//span[@class="article-list-date"]/text()''').text ().strip ()
            date = str(date).replace("-", "")


        # for title, link, date in zip(titles, links, dates):
            link = "http://www.fjggzyjy.cn" + str(link)
    ...
    def getdumps(self, url):
        if self.conn.sismember("dumps:fjggzyjy", url):
            return True  # 在里面
        else:
            print "入redis：去重库"
            self.conn.sadd("dumps:fjggzyjy", url)
            return False
...
if __name__ == '__main__':
    ...
    url = "http://www.fjggzyjy.cn/news/category/10/"
