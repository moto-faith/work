...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '国泰君安期货'
        # 网站一级域名
        self.site_domain = 'gtjaqh.com'

        self.start_urls = [
            # 列表页链接
            'http://gtjaqh.com/gtqh/index/informat/more?id=4',#国内新闻
            'http://gtjaqh.com/gtqh/index/informat/more?id=3',#国际新闻
            'http://gtjaqh.com/gtqh/index/informat/more?id=5',#时事要闻
            'http://gtjaqh.com/gtqh/index/informat/more?id=2',#宏观经济
            'http://gtjaqh.com/gtqh/index/informat/more?id=186',#市场评论
            'http://gtjaqh.com/gtqh/index/informat/more?id=187',#最新动态
            'http://gtjaqh.com/gtqh/index/informat/more?id=185',#全球股市
            'http://gtjaqh.com/gtqh/index/informat/more?id=175',#现货市场
            'http://gtjaqh.com/gtqh/index/informat/more?id=176',#市场评论
            'http://gtjaqh.com/gtqh/index/informat/more?id=178',#机构报告
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="so_list"]//li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://gtjaqh.com',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        title = data.xpath('''//div[@class="news_titlef"]/h3//text()''').text().strip()

        gtime = datetime.datetime.utcnow()
        # 详情页内解析包含发布时间的模块，一般以//span、//div、//p结尾
        # ctime = data.xpath('''//div[@class="layout"]//div[@class="left"]''')

        ctime = data.xpath('''//div[@class="news_titlef"]/div[@class="news_timef"]/text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        content1 = data.xpathall('''//div[@class="news_textf"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="news_textf"]//img/@src''')
        if pic_urls_list:
