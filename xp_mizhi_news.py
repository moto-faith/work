...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '米脂县人民政府'
        # 网站一级域名
        self.site_domain = 'mizhi.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.mizhi.gov.cn/news_list.rt?channlId=1',# 新闻中心
            'http://www.mizhi.gov.cn/html/xwzx/mzyw/index.html',#通知公告
            'http://www.mizhi.gov.cn/html/xwzx/xzbm/index.html',#部门动态
            'http://www.mizhi.gov.cn/html/xwzx/tzgg/index.html',  #乡镇动态
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="nylist"]//li/a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.mizhi.gov.cn', post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        source =data.xpath('''//span[@id="origin"]//text()''').text().strip() or self.siteName
