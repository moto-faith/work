...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '湖南车市'
        # 网站一级域名
        self.site_domain = 'hncheshi.com'

        self.start_urls = [
            # 列表页链接
            'http://www.hncheshi.com/news/localnews.html',#  湖南车市
            'http://www.hncheshi.com/news/lowerprices.html',#优惠•促销
            'http://www.hncheshi.com/news/factory.html',#品牌新闻
            'http://www.hncheshi.com/news/interview.html',  #高端访谈
            'http://www.hncheshi.com/news/internal.html',  #新车上市
            'http://www.hncheshi.com/news/evaluating.html',  #新车•评测
            'http://www.hncheshi.com/news/industry.html',  #行业新闻
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@id="tablelsw"]//a''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//span[@class="pubTime"]''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel =data.xpath('''//div[@class="mingcheng"]/a[2]//text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@id="Cnt-Main-Article"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="Cnt-Main-Article"]//img/@src''')
        if pic_urls_list:
