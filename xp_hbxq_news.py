...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '宿迁湖滨新区管理委员会'
        # 网站一级域名
        self.site_domain = 'hbxq.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://hbxq.gov.cn/hbxq/zwxw/list.shtml',# 政务要闻
            'http://hbxq.gov.cn/hbxq/bmdt/list.shtml',#部门动态
            'http://hbxq.gov.cn/hbxq/xzdt/list.shtml',#乡镇动态
            'http://hbxq.gov.cn/hbxq/mtjj/list.shtml',#媒体聚焦
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="list_r_lb_h"]/table//a''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = "http://hbxq.gov.cn"+ post_url[5:]
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source = data.xpath('''//td[6]/span//text()''').text().strip() or self.siteName
