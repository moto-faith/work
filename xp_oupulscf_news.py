...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '中国供应链交易中心'
        # 网站一级域名
        self.site_domain = 'oupulscf.com'

        self.start_urls = [
            # 列表页链接
            'http://www.oupulscf.com/content/list.htm?catCode=A001',#厂家动态
            'http://www.oupulscf.com/content/list.htm?catCode=A002',#行业资讯
            'http://www.oupulscf.com/content/list.htm?catCode=A003',# 国际行情
            'http://www.oupulscf.com/content/list.htm?catCode=A004',#政策法规
            'http://www.oupulscf.com/content/list.htm?catCode=A005',# 知识百科
            'http://www.oupulscf.com/content/list.htm?catCode=A006',#资讯
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.oupulscf.com',post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        content1 = data.xpathall('''//div[@id="frameContent"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="frameContent"]//img/@src''')
        if pic_urls_list:
