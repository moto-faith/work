...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '泉州房产网'
        # 网站一级域名
        self.site_domain = 'hxdctz.com'

        self.start_urls = [
            # 列表页链接
            'http://www.hxdctz.com/a/meiritoutiao/',# 每日头条
            'http://www.hxdctz.com/a/loushikuaixun/',#楼市快讯
            'http://www.hxdctz.com/a/dujiabaodao/',#独家报道
            'http://www.hxdctz.com/a/gongchengjindu/',  #工程进度
            'http://www.hxdctz.com/a/huodongyugao/',  #活动预告
            'http://www.hxdctz.com/a/haixizhuanfang/',  #海西专访
            'http://www.hxdctz.com/a/tudipaimai/',  #土地拍卖
            'http://www.hxdctz.com/a/tuangouyouhui/',  #团购优惠
            'http://www.hxdctz.com/a/loupanyouhui/',  #楼盘优惠
            'http://www.hxdctz.com/a/quanzhouxinwen/',  #泉州新闻
            'http://www.hxdctz.com/a/haixifangchan/',  #海西房产
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="news_list clearfix"]/ul/li/a''')
            # self.channel = data.xpath('''//div[@class="page-title"]//text()''').text().strip()
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin('http://www.hxdctz.com', post_url)
    ...
    def get_detail_page_info(self, data):
        ...
        retweeted_source =data.xpath('''//div[@class="tit_bar_text"]/text()[2]''').text().strip() or self.siteName
        ...
        content1 = data.xpathall('''//div[@class="news_content"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@class="news_content"]//img/@src''')
        if pic_urls_list:
