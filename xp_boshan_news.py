...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '博山政务网'
        # 网站一级域名
        self.site_domain = 'boshan.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.boshan.gov.cn/col/col6476/index.html',# 图片新闻
            'http://www.boshan.gov.cn/col/col4423/index.html',#政务要闻
            'http://www.boshan.gov.cn/col/col4424/index.html',#部门动态
            'http://www.boshan.gov.cn/col/col4425/index.html',  #镇办动态
            'http://www.boshan.gov.cn/col/col4428/index.html',  #通知公告
            'http://www.boshan.gov.cn/col/col6705/index.html',  #省政府信息
            'http://www.boshan.gov.cn/col/col6760/index.html',  #市政府信息
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            # loops = data.xpathall('''//div[@class="lm_right_list"]/ul/li/a''')

            # for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
            for item in re.findall('''href="(/art.*?)"''',data.response.text):
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//td[@class='wz_time']//text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        content1 = data.xpathall('''//td[@class="bt_content"]''')
        ...
        pic_urls_list = data.xpathall('''//td[@class="bt_content"]//img/@src''')
        if pic_urls_list:
