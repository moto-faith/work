...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '	达州新闻网'
        # 网站一级域名
        self.site_domain = 'nigame.com.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.nigame.com.cn/xinwen/shizheng/', #时政资讯
            'http://www.nigame.com.cn/xinwen/guonei/',  # 国内资讯
            'http://www.nigame.com.cn/xinwen/guoji/',  # 国际资讯
            'http://www.nigame.com.cn/xinwen/fangtan/',  # 访谈资讯
            'http://www.nigame.com.cn/xinwen/zhengwu/', # 政务资讯
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//div[@class="imgr"]/h2''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="infor"]/p[@class="publish_time"]''').regex('\d+-\d+-\d+ \d+:\d+:\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        ...
        retweeted_source = data.xpath('''//div[@class="infor"]/p[@class="resource"]//text()''').text().strip() or  self.siteName
