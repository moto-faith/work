...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.info_flag = "01"
        # 网站名称
        self.siteName = '淮北市人力资源和社会保障局'
        # 网站一级域名
        self.site_domain = 'hbshrss.gov.cn'

        self.start_urls = [
            # 列表页链接
            'http://www.hbshrss.gov.cn/Category_109/Index.aspx', #工作动态
            'http://www.hbshrss.gov.cn/Category_144/Index.aspx',  # 新闻中心
            'http://www.hbshrss.gov.cn/Category_133/Index.aspx',  # 工作动态
            'http://www.hbshrss.gov.cn/Category_135/Index.aspx', #图片新闻
            'http://www.hbshrss.gov.cn/Category_303/Index.aspx',#规范性文件
            'http://www.hbshrss.gov.cn/Category_235/Index.aspx' #政策解读
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
        self.encoding = 'utf-8'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//table[@class="contentList"]//tr''')
            ...
            for item in loops:  # 这时的item可以看做将解析到的模块当做新的网页打开，页面内只有解析到的模块，以解析的模块为顶级节点进行解析，而不再是标签
                ...
                if not post_url:
                    continue
                post_url = urljoin("http://www.hbshrss.gov.cn",post_url)
