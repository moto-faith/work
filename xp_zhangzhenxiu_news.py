...
class MySpider(spiderDefault.Spider):
    ...
    def __init__(self, cmd_args=None):
        ...
        self.siteName = '句容新闻网'
        # 网站一级域名
        self.site_domain = 'zhangzhenxiu.com.cn'

        self.start_urls = [

            # 列表页链接
            'http://zhangzhenxiu.com.cn/news/',
        ]
        # 网页编码
        # 例：self.encoding = 'gbk'
    ...
    def get_detail_page_urls(self, data):
        ...
        if data is not None:
            ...
            loops = data.xpathall('''//ul[@class="newslist14px"]/li''')
    ...
    def get_detail_page_info(self, data):
        ...
        ctime = data.xpath('''//div[@class="sourcetime"]//text()''').regex('\d+-\d+-\d+').datetime()
        # ctime = gtime
        if ctime < self.c_time:
            return None
        # 详情页解析当前页面所属频道的面包屑，一般以//text()结尾
        # source = data.xpath('''//div[@class="add"]//a[last()]//text()''')
        channel = data.xpath('''//div[@class="qtree"]/a[2]//text()''').text().strip()
        ...
        content1 = data.xpathall('''//div[@id="endText"]''')
        ...
        pic_urls_list = data.xpathall('''//div[@id="endText"]//img/@src''')
        if pic_urls_list:
